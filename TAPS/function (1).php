	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
		...
		if (isset($_POST['review'])) {
			...
			if ($_POST["review"] == "CURRENT"){
				print '<h2>Filter Result - CURRENT</h2>';
			}

			if ($_POST["review"] == "RAW"){
				print '<h2>Filter Result - RAW</h2>';
			}

			if ($_POST["review"] == "HISTORY"){
				print '<h2>Filter Result - HISTORY</h2>';
			}
...
*/		   
			if ($_POST["review"]=="CURRENT"){
				...
				glab_curr.sample_id AS 'Sample ID',glab_curr.compound_group as 'Group',
				glab_curr.compound_code as 'Compound',glab_curr.conc_ppbv as 'Conc.(ppbv)/SAMPLE',
				glab_curr.conc_mcg_m3 as GLabResult, contractor_curr.flow_rate as 'Flow(LPM)',contractor_curr.duration as 'Time(min)',
				(contractor_curr.flow_rate/1000)*contractor_curr.duration AS 'Volume(m3)',
				IF (glab_curr.conc_ppbv < 0,
					(glab_curr.conc_ppbv/-2/((contractor_curr.flow_rate/1000)*contractor_curr.duration)),
					(glab_curr.conc_ppbv/((contractor_curr.flow_rate/1000)*contractor_curr.duration)) ) AS 'Conc/M3',
			...
			if ($_POST["review"]=="RAW"){
				...
				glab_template.conc_mcg_m3 as GLabResult, contractor_template.flow_rate as 'Flow(LPM)',contractor_template.duration as 'Time(min)',
	...
	//stp				while ($r_rev = $glab_rs->fetch_array(MYSQLI_BOTH)) {
					...
					while ($r_rev = mysqli_fetch_assoc($resultunt)) {
						if ($_POST["review"]=="CURRENT"){
							...
							<td style="width:30px"><input type="text" id="site.'.$i.'" name="site[]" readonly value='.$r_rev['Site'].' style="width:100%"></td>
							<input type="hidden" id="rwstatus.'.$i.'" name="rwstatus.'.$i.'" value="NNNNNNNNNNNNNNN"></td>

							<td style="width:60px"><input type="text" id="s_date.'.$i.'" name="s_date[]" readonly value='.$r_rev['Start Date'].' style="width:100%"></td>
							<td style="width:90px"><input type="text" id="sample_id.'.$i.'" name="sample_id[]" readonly value='.$r_rev['Sample ID'].' style="width:100%"></td>				
							<td style="width:30px"><input type="text" id="group.'.$i.'" name="group[]" readonly value='.$r_rev['Group'].' style="width:100%"></td>
							<td style="width:220px"><input type="text" id="compound.'.$i.'" name="compound[]" readonly value='.$r_rev['Compound'].' style="width:100%"></td>
							<td style="width:30px"><input type="text" id="glabresult.'.$i.'" name="glabresult[]" readonly value='.$r_rev['GLabResult'].' style="width:100%"></td>			
							<td style="width:100px"><input type="number" step="0.01" id="sample_conc.'.$i.'" name="sample_conc[]" value="'.$r_rev["Conc.(ppbv)/SAMPLE"].'" style="width:98%">
							<input type="hidden" id="Original_sample_conc.'.$i.'" name="Original_sample_conc.'.$i.'" value='.$r_rev["Conc.(ppbv)/SAMPLE"].'></td>

							<td style="width:80px"><input type="text" id="conc_m3.'.$i.'" name="conc_m3[]" readonly value='.round($r_rev['Conc/M3'],5).' style="width:100%"></td>							
							
							<td style="width:80px"><input type="number" step="0.01" id="flow.'.$i.'" name="flow[]" value="'.$r_rev["Flow(LPM)"].'" style="width:100%">					
							<input type="hidden" id="Original_flow.'.$i.'" name="Original_flow.'.$i.'" value='.$r_rev["Flow(LPM)"].'></td>

							<td style="width:80px"><input type="number" step="0.01" id="mtime.'.$i.'" name="mtime[]" value="'.$r_rev["Time(min)"].'" style="width:100%">
							<input type="hidden" id="Original_mtime.'.$i.'" name="Original_mtime.'.$i.'" value="'.$r_rev["Time(min)"].'"></td>

							<td style="width:80px"><input type="number" step="0.01" id="vol.'.$i.'" name="vol[]" readonly value='.round($r_rev["Volume(m3)"],5).' style="width:100%"></td>

							<td style="width:80px"><input type="number" step="0.01" id="who_tef.'.$i.'" name="who_tef[]" value='.$r_rev["WHO TEF Ratio"].' style="width:100%">
							<input type="hidden" id="Original_who_tef.'.$i.'" name="Original_who_tef.'.$i.'" value='.$r_rev["WHO TEF Ratio"].'></td>

							<td style="width:80px"><input type="text" id="who_teq.'.$i.'" name="who_teq[]" readonly value="'.$r_rev["WHO TEQ Conc/m3"].'" style="width:100%"></td>
							<!-- <input type="hidden" id="Original_who_teq.'.$i.'" name="Original_who_teq.'.$i.'" value='.round($r_rev["WHO TEQ Conc/m3"],5).'> -->

							<td style="width:80px"><input type="text"  id="casno1.'.$i.'" name="casno1[]" value='.$r_rev['casno1'].' style="width:95%">						
							<input type="hidden" id="Original_casno1.'.$i.'" name="Original_casno1.'.$i.'" value='.round($r_rev["casno1"],5).'></td>
						...
						if ($_POST["review"]=="RAW"){
							print '<tr id='.$i.' class='.$i.'>
							<td style="width:30px"><input type="text" id="site.'.$i.'" name="site[]" readonly value='.$r_rev['Site'].' style="width:100%"></td>
							<td style="width:120px"><input type="text" id="s_date.'.$i.'" name="s_date[]" readonly value='.$r_rev['Start Date'].' style="width:100%"></td>
							<td style="width:130px"><input type="text" id="sample_id.'.$i.'" name="sample_id[]" readonly value='.$r_rev['Sample ID'].' style="width:100%"></td>				
							<td style="width:50px"><input type="text" id="group.'.$i.'" name="group[]" readonly value='.$r_rev['Group'].' style="width:100%"></td>
							<td style="width:50px"><input type="text" id="compound.'.$i.'" name="compound[]" readonly value='.$r_rev['Compound'].' style="width:100%"></td>
							<td style="width:20px"><input type="text" id="glabresult.'.$i.'" name="glabresult[]" readonly value='.$r_rev['GLabResult'].' style="width:100%"></td>
							<td style="width:80px"><input type="number" id="sample_conc.'.$i.'" name="sample_conc[]" readonly value='.$r_rev["Conc.(ppbv)/SAMPLE"].' style="width:100%"></td>
							<td style="width:80px"><input type="number" id="conc_m3.'.$i.'" name="conc_m3[]" readonly value='.round($r_rev['Conc/M3'],5).' style="width:100%"></td>							
							<td style="width:80px"><input type="number" id="flow.'.$i.'" name="flow[]" readonly value="'.$r_rev["Flow(LPM)"].'" style="width:100%"></td>					
							<td style="width:80px"><input type="number" id="mtime.'.$i.'" name="mtime[]" readonly value='.$r_rev["Time(min)"].' style="width:100%"></td>
							<td style="width:80px"><input type="number" id="vol.'.$i.'" name="vol[]" readonly value='.round($r_rev["Volume(m3)"],5).' style="width:100%"></td>
							<td style="width:80px"><input type="number" id="who_tef.'.$i.'" name="who_tef[]" readonly value='.$r_rev["WHO TEF Ratio"].' style="width:100%"></td>
							<td style="width:80px"><input type="number" id="who_teq.'.$i.'" name="who_teq[]" readonly value='.round($r_rev["WHO TEQ Conc/m3"],5).' style="width:100%"></td>
							<td style="width:80px"><input type="text"  id="casno1.'.$i.'" name="casno1[]" readonly value='.$r_rev['casno1'].' style="width:100%"></td>						
							</tr>';
						}
						...
						if ($_POST["review"]=="HISTORY"){
							...
							print '<tr id='.$i.' class='.$i.'>
							<td style="width:20px"><input type="text" id="site.'.$i.'" name="site[]" readonly value="'.$r_rev['Site'].'" style="width:100%"></td>
							<td style="width:50px"><input type="text" id="s_date.'.$i.'" name="s_date[]" readonly value="'.$r_rev['Start Date'].'" style="width:100%"></td>
							<td style="width:70px"><input type="text" id="sample_id.'.$i.'" name="sample_id[]" readonly value="'.$r_rev['Sample ID'].'" style="width:100%"></td>				
							<td style="width:20px"><input type="text" id="group.'.$i.'" name="group[]" readonly value="'.$r_rev['Group'].'" style="width:100%"></td>
							<td style="width:50px"><input type="text" id="compound.'.$i.'" name="compound[]" readonly value="'.$r_rev['Compound'].'" style="width:100%"></td>
							<td style="width:50px"><input type="text" id="glabresult.'.$i.'" name="glabresult[]" readonly value="'.($r_rev['GLab Result']=="" ? 'NULL':$r_rev['GLab Result']).'" style="width:100%"></td>
							<td style="width:80px"><input type="text" id="sample_conc.'.$i.'" name="sample_conc[]" readonly value="'.$r_rev["Conc.(ppbv)/SAMPLE"].'" style="width:100%"></td>
							<td style="width:60px"><input type="text" id="conc_m3.'.$i.'" name="conc_m3[]" readonly value="'.round($r_rev['Conc/M3'],5).'" style="width:100%"></td>							
							<td style="width:30px"><input type="text" id="flow.'.$i.'" name="flow[]" readonly value="'.$r_rev["Flow(LPM)"].'" style="width:100%"></td>				
							<td style="width:30px"><input type="text" id="mtime.'.$i.'" name="mtime[]" readonly value="'.$r_rev["Time(min)"].'" style="width:100%"></td>
							<td style="width:30px"><input type="text" id="vol.'.$i.'" name="vol[]" readonly value="'.round($r_rev["Volume(m3)"],5).'" style="width:100%"></td>
							<td style="width:80px"><input type="text" id="who_tef.'.$i.'" name="who_tef[]" readonly value="'.$r_rev["WHO TEF Ratio"].'" style="width:100%"></td>
							<td style="width:30px"><input type="text" id="who_teq.'.$i.'" name="who_teq[]" readonly value="'.round($r_rev["WHO TEQ Conc/m3"],5).'" style="width:100%"></td>
							<td style="width:80px"><input type="text"  id="casno1.'.$i.'" name="casno1[]" readonly value="'.$r_rev['casno1'].'" style="width:100%"></td>						
							<td style="width:80px"><input type="text"  id="rmk.'.$i.'" name="rmk[]" readonly value="'.($r_rev['Remark']=="" ? 'NULL':$r_rev['Remark']).'" style="width:100%"></td>						
							<td style="width:80px"><input type="text"  id="modi_dt.'.$i.'" name="modi_dt[]" readonly value="'.$mdt.'" style="width:100%"></td>
...
*/		
		...
		$(document).ready(function(){		  
		  $(\"input\").change(function (e) {
			  ...
			  alert('Attr Id = ' + $(this).attr('id').slice(0,4));	
			  alert('rowId = ' + rowId);

				<!--Assign a value-->
			  <!--$(\"input[id='abc']\").val(\"Glenn Quagmire\"); -->			  

			  <!-- Return the current value-->
			  <!--$(this).val()-->

			  <!-- if ($(this).attr('id').slice(0,4)==='flow'){ -->
					$(\"input[id='vol.\" + rowId + \"']\").val($(this).val() / 1000 * $(\"input[id='mtime.\" + rowId + \"']\").val() );
			  <!-- } -->

			  alert ('sample_conc = ' + $(\"input[id='sample_conc.\" + rowId + \"']\").val());
			  if ($(\"input[id='sample_conc.\" + rowId + \"']\").val() < 0){				  			  
				$(\"input[id='conc_m3.\" + rowId + \"']\").val( $(\"input[id='sample_conc.\" + rowId + \"']\").val() / -2 / $(\"input[id='vol.\" + rowId + \"']\").val()  );
				}

	  		  if ($(\"input[id='sample_conc.\" + rowId + \"']\").val() > 0){				  			  
				$(\"input[id='conc_m3.\" + rowId + \"']\").val( $(\"input[id='sample_conc.\" + rowId + \"']\").val() / $(\"input[id='vol.\" + rowId + \"']\").val()  );
				}
	

			  if ($(\"input[id='sample_conc.\" + rowId + \"']\").val() < 0){				  			  
				$(\"input[id='who_teq.\" + rowId + \"']\").val( $(\"input[id='sample_conc.\" + rowId + \"']\").val() / -2 / $(\"input[id='vol.\" + rowId + \"']\").val() * $(\"input[id='who_tef.\" + rowId + \"']\").val() );
				}

			  if ($(\"input[id='sample_conc.\" + rowId + \"']\").val() > 0){				  			  
				$(\"input[id='who_teq.\" + rowId + \"']\").val( $(\"input[id='sample_conc.\" + rowId + \"']\").val() / $(\"input[id='vol.\" + rowId + \"']\").val() * $(\"input[id='who_tef.\" + rowId + \"']\").val() );
				}
	

				<!-- $(\"input[id='conc_m3.\" + rowId + \"']\").val(333888); -->
