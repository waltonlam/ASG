...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
	...
	//*********Create T**************/	
		...
		for($i = 0; $i < count($_POST['id']); $i++) {			
			...
			$d.=$rn1.$i.";".$T.$i." (INDEX USING BTREE(loc_id, sample_date, m_time)) Select loc_id,sample_date,m_time,sample_v as '".$_POST['id'][$i]
	...
	//*****Restrcture Left&Right************
		...
		if (count($_POST['id']) > 1){
			...
			$J="create table A".$_SESSION['o'].$i." ( loc_id  varchar(5),sample_date date DEFAULT NULL,m_time time DEFAULT NULL, INDEX USING BTREE(loc_id, sample_date, m_time) ) select T".$_SESSION['o'].$i.".loc_id,T".$_SESSION['o'].$i."
			...
			for($i = 1; $i < count($_POST['id'])-1; $i++) {		
				$rn2 .= "drop table if exists A".$_SESSION['o'].$i.";";
				$J .= "create table A".$_SESSION['o'].$i." ( loc_id  varchar(5),sample_date date DEFAULT NULL,m_time time DEFAULT NULL, INDEX USING BTREE(loc_id, sample_date, m_time)) select A".$_SESSION['o'].($i-1).".loc_id,A".$_SESSION['o'].($i-1)."
...
$h = "select loc_id,gcde,MIN(sample_date) AS from_date,MAX(sample_date) AS to_date FROM sample_test use index (loc_gcde)
...
//Frame
echo '<table style="width:100%;margin-left:0%;"><td style="background-color:rgb(178,210,218);vertical-align:top;width:16%;">Data Availability:-</p>';
...
while ($r_da = $result_da->fetch_array(MYSQLI_BOTH)) {
	...
	print '<td style="background-color:rgb(100,210,218);vertical-align:top;width:15%;">'.$r_da["loc_id"].'</td>';
	print '<td style="background-color:rgb(200,210,218);vertical-align:top;width:25%;">'.$r_da["gcde"].'</td>';
	print '<td style="background-color:rgb(100,310,218);vertical-align:top;width:30%;">'.$r_da["from_date"].'</td>';
	print '<td style="background-color:rgb(100,510,218);vertical-align:top;width:30%;">'.$r_da["to_date"].'</td></tr>';
			
}

print '</table>
</td><td style="background-color:rgb(178,210,218);vertical-align:top;">';
