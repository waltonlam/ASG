...
session_start();
define('TITLE', 'Delete Parameter Group');
...
print '<script type="text/javascript">
    document.getElementById("cancel").onclick = function GoBack() {
	...
	if (!empty($_POST['lid']))	
		{
				...
				$u="select * from element" ." where gcde='".$_POST['lid']."' limit 1";
				$ck_g=$dbc->query($u);
				if ($ck_g->num_rows){
				print '<p class="text--error">'.'Parameter group cannot be deleted! There are records related to this group. Please seek help from administrator</p>
				<br></br><p></p><span style="color:blue;font-weight:bold;padding-left:35px;padding-right:15px"><a href="sampletx.php">Home</a></span>';
				exit();
				}

				$u= "delete from eleg"
				." where gcde='".$_POST['lid']."';";

				//$dbc->autocommit(FALSE);

					if ($dbc->query($u) === TRUE) {
					   // $updated=TRUE;
					    echo '<p style="color:blue;font-weight:bold;text-align:center;font-size:1cm">Parameter group has been deleted</p>
					    <br></br><p></p><span style="color:blue;font-weight:bold;padding-left:35px;padding-right:15px"><a href="sampletx.php">Home</a></span>';
...
print '
<form action="del_gp.php" method="post">
			<p style="text-align:center;font-size:28px;color:red;font-weight:bold;">Delete Parameter Group</p>
				...
				<td style="width:48%;text-align:right">	
					...
					<select style="margin-left:10px" name="lid">';
					   while ($r_l=$result_g->fetch_object()){
						  if ($r_l->lid==$t[0]){
							  print '<option value="'.$r_l->lid.'" selected>'.'('.$r_l->lid.') '.$r_l->gname.'</option>';}
						  else{
							 print '<option value="'.$r_l->lid.'">'.'('.$r_l->lid.') '.$r_l->gname.'</option>';}
					};					
