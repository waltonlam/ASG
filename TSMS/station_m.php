...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
	if (!empty($_POST['sid'])) 
	{
				...
				$criteria = 'CGS: '.$_POST['sid'];
				if ($_POST['sid']=='ALL'){
					$station_id= " and ud.userid='".$_SESSION['vuserid']."'";
				}else{
					$station_id=" and ud.userid='".$_SESSION['vuserid']."' and s.station_id = '".$_POST['sid']."' ";
				}
...
*/
					...
					$q = "select s.name sname, s.district_id did, s.station_id sid, s.address saddr"
					." from station s, user_district ud where ud.district_id=s.district_id ".$station_id." order by s.station_id;";	
...
print '	
<form action="station_m.php" method="post">
    <table style="width:100%;margin-left:0%;">
		<th style="background-color:#008F00;color:yellow;font-weight:bold;font-size:28px">CGS資料維護</th>
  ...
  <select name="sid" style="font-size:18px">
    ...
    echo '<table style="width:100%;margin-left:0%;">
			...
			if (!empty($criteria)){ 
					while ($r_st=$result_st->fetch_object()){
	    					...
	    					.'<td style="background-color:#00B386;color:white"><a href="update_st_m.php?cluster='.$r_st->did.'|'.$r_st->sid.'|'.$r_st->sname.'|'.$r_st->saddr.'">'.$r_st->did.'</a></td>'
	    					.'</td><td id="trans_date" style="background-color:#00B386;color:white">'.$r_st->sid.'</td><td style="background-color:#00ADB3;color:white">'.$r_st->sname.'</td>'
	    					.'<td style="background-color:#00ADB3;color:white">'.$r_st->saddr.'</td>'
								.'<td style="background-color:#B5B5B5"><a href="del_st_m.php?cluster='.$r_st->did.'|'.$r_st->sid.'|'.$r_st->sname.'|'.$r_st->saddr.'"><img src="img/del.png"  width="20" height="20" value='.$r_st->sid.'></a></td></tr>';
...
print'
  ...
  <span style="color:blue;font-weight:bold;"><a href="station_addnew.php">新增CGS</a></span>
';
