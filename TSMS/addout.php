...
print '<script>
		...
		document.getElementById("prg_id").load = function() {myFunction_a()};

		function myFunction_a() {
		    document.getElementById("prg_id").innerHTML = "Iframe is loaded.";
			alert("hello");		    
		}
</script>';
...
if (empty($_SESSION['vuserid'])) {
		...
		exit();
					...
					$q="select * from prg_cat order by prg_id;";
					$result_pc=$dbc->query($q);
					if (!$result_pc->num_rows){
							print '<p class="text--error">Program Category Configuration Empty! Please configure it before try again.</p>';
							exit();				 						 
						 }
					$o="select o.outlet_id oid, o.name oname, op.prg_id op_id, pc.name pc_name from out_prg op, prg_cat pc, outlet o where op.outlet_id=o.outlet_id and op.prg_id=pc.prg_id and pc.prg_id='";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') 
{
	if (!empty($_POST['trans_date']) and !empty($_POST['qty']) and !empty($_POST['unit_price']) and !empty($_POST['recp_no'])) 	
	{
				...
				values ('".date('Y/m/d', strtotime($_POST['trans_date']))."','".$_POST['district_id']
						."','".$_POST['prg_id']."','".$_POST['new_outlet_prg']."',".$_POST['qty']
...
<form onsubmit="submitForm()" action="addout.php" method="post">
			<p id="abc" style="text-align:center;font-size:28px;color:blue;font-weight:bold;">新增送往下游回收商記錄</p>
...
print	'</td><tr style="color:#555555;">
			...
			<tr style="color:#555555;">
				...
				<td><select style="margin-left:10px"  name="prg_id" onchange="showprg(this.value)" >';
					$first_pc="";
				 	while ($r_pc=$result_pc->fetch_object()){
				 		if ($first_pc==""){
				 			$first_pc=$r_pc->prg_id;
				 			}
...
print '</select></td>				 	
				...
				<td style="width:48%;text-align:right">
					<label for="new_outlet_prg">回收商名稱</label>
				</td>
				<td>';

		$o=$o.$first_pc."'";
		$result_o=$dbc->query($o);
		if (!$result_o->num_rows){
				print '<p class="text--error">Outlet Configuration Empty! Please configure it before try again.</p>';
				exit();
		}else{			
				print '<select style="margin-left:10px" name="new_outlet_prg" id="new_outlet_prg">';
				while ($r_ou=$result_o->fetch_object()){
				    print '<option value="'.$r_ou->oid.'">'.'('.$r_ou->oid.') '.$r_ou->oname.'</option>';
				  }			
				print '</select>';
				}
