...
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	...
	if ($_POST['sortord']=="loc"){ 
		$h = "select loc_id,gcde,MIN(sample_date) AS from_date,MAX(sample_date) AS to_date FROM sample_ele use index (loc_gcde)
		group by loc_id,gcde order by loc_id,gcde;";
	}

	if ($_POST['sortord']=="gcde"){
		$h = "select loc_id,gcde,MIN(sample_date) AS from_date,MAX(sample_date) AS to_date FROM sample_ele use index (gcde_loc)
		group by gcde,loc_id order by gcde, loc_id;";
	}
}
else{
	$h = "select loc_id,gcde,MIN(sample_date) AS from_date,MAX(sample_date) AS to_date FROM sample_ele use index (gcde_loc)
...
<tr><fieldset><legend style="font-weight:bold">Ordering</legend>';
if ($_SERVER['REQUEST_METHOD'] == 'POST'){
	if ($_POST['sortord']=="loc"){ 
		print '<input type="radio" id="sortloc" name="sortord" onchange="seleC()" value="loc" checked>';
	}else{
		print '<input type="radio" id="sortloc" name="sortord" onchange="seleC()" value="loc">';
	}	
	print '<label for="sortloc">Location     </label>';

	if ($_POST['sortord']=="gcde"){ 
		print '<input type="radio" id="sortgcde" name="sortord" onchange="seleC()" value="gcde" checked>';
	}else{
		print '<input type="radio" id="sortgcde" name="sortord" onchange="seleC()" value="gcde">';
	}
	print '<label for="sortgcde">Parameter Group</label></tr>';

}else{
	print '<input type="radio" id="sortloc" name="sortord" onchange="seleC()" value="loc"><label for="sortloc">Location     </label>
			<input type="radio" id="sortgcde" name="sortord" onchange="seleC()" value="gcde" checked><label for="sortgcde">Parameter Group</label></fieldset></tr>';
}
