...
{
	if (!empty($_POST['uid']) and !empty($_POST['pwd']) and !empty($_POST['fname']) and !empty($_POST['lname']) and !empty($_POST['ronly']))	
		{
				$u= "update uacc set pwd='".$_POST['pwd']."',fname='".$_POST['fname']."',lname='".$_POST['lname']."',ronly='".$_POST['ronly']."'"
				." where uid='".$_POST['uid']."';";
				...
				//$dbc->autocommit(FALSE);
					...
					if ($dbc->query($u) === TRUE) {
					   // $updated=TRUE;
					    echo '<p style="color:blue;font-weight:bold;text-align:center;font-size:1cm">User Account has been updated</p>
...
from uacc;";
//print $q;
$result_uac=$dbc->query($l);
if (!$result_uac->num_rows){
...
print '
<form action="update_ua_m.php" method="post">
			<p style="text-align:center;font-size:28px;color:blue;font-weight:bold;">Update User Account Info.</p>
...
print '<tr style="color:#555555;">
				<td style="width:48%;text-align:right">	
					...
					<td>
					<select style="margin-left:10px" name="uid">';
					   while ($r_l=$result_uac->fetch_object()){
						  if ($r_l->uid==$t[0]){
							  print '<option value="'.$r_l->uid.'" selected>('.$r_l->uid.') '.$r_l->fname.' '.$r_l->lname.' - '.$r_l->ronly.'</option>';}
						  else{
								print '<option value="'.$r_l->uid.'">('.$r_l->uid.') '.$r_l->fname.' '.$r_l->lname.' - '.$r_l->ronly.'</option>';}
					};					
...
print '</td></tr>			
			...
			<tr style="color:#555555;">
				<td style="width:48%;text-align:right">
					<label for="sid">Access</label>
				</td>
				<td>
				  <select style="margin-left:10px" name="ronly">
