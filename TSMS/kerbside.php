...
print '
<form action="kerbside.php" method="post">
    <table style="width:100%;margin-left:0%;">
		<th style="background-color:#008F00;color:yellow;font-weight:bold;font-size:28px">街站回收記錄</th>
  <span style="padding-right:1cm"></span>
  </table>
  <div column=2>
  <span style="padding-right:1cm"></span>
  <span style="align:right;color:blue;font-weight:bold;font-size:18px">街站位置</span>
  <select name="kerbside" style="font-size:18px">
...
//欄位header
    echo '<table style="width:100%;margin-left:0%;">
			...
			<th style="background-color:#008F00;color:white">街站位置(名稱)</th>
			...
			if (!empty($criteria)){ 
					while ($r_kbs=$result_kbs->fetch_object()){
						    //printf("%s (%s)\n",$obj->Lastname,$obj->Age);
	    					print '<tr style="text-align:center;color:#555555;"><td style="background-color:#00B386;color:white">'.$r_kbs->t_trans_no
	    					
	    					//.'</td><td style="background-color:#00B386;color:white">'.$r_kbs->trans_date.'</td>'
	    					
	    					.'<td style="background-color:#00B386;color:white"><a href="update.php?cluster='.$r_kbs->t_trans_no.'|'.$r_kbs->t_trans_date.'|'.$r_kbs->t_source_id.'|'.$r_kbs->kname.'|'.$r_kbs->t_glass_bottle_qty.'|'.$r_kbs->t_ee_qty.'|'.$r_kbs->t_comp_qty.'|'.$r_kbs->t_battery_qty.'|'.$r_kbs->t_light_qty.'|'.$r_kbs->t_paper_qty.'|'.$r_kbs->t_plastic_qty.'|'.$r_kbs->t_metal_qty.'|'.$r_kbs->t_toner_qty.'|'.$r_kbs->t_clothes_qty.'|'.$r_kbs->t_book_qty.'|'.$r_kbs->t_toy_qty.'|'.$r_kbs->t_other_qty.'|'.$r_kbs->t_other_desc.'">'.$r_kbs->t_trans_date.'</a></td>'
	    					.'<td style="background-color:#00B386;color:white">'.$r_kbs->t_source_id.'</td><td style="background-color:#00B386;color:white">'.$r_kbs->kname.'</td><td style="background-color:#00ADB3;color:white">'.$r_kbs->t_glass_bottle_qty.'</td>'
	    					.'<td style="background-color:#00ADB3;color:white">'.$r_kbs->t_ee_qty.'</td><td style="background-color:#00ADB3;color:white">'.$r_kbs->t_comp_qty.'</td><td style="background-color:#00ADB3;color:white">'.$r_kbs->t_battery_qty.'</td>'
	    					.'<td style="background-color:#00ADB3;color:white">'.$r_kbs->t_light_qty.'</td><td style="background-color:#EB6E80;color:white">'.$r_kbs->t_paper_qty.'</td><td style="background-color:#EB6E80;color:white">'.$r_kbs->t_plastic_qty.'</td>'
	    					.'<td style="background-color:#EB6E80;color:white">'.$r_kbs->t_metal_qty.'</td><td style="background-color:#EB9900;color:white">'.$r_kbs->t_toner_qty.'</td><td style="background-color:#EB9900;color:white">'.$r_kbs->t_clothes_qty.'</td>'
	    					.'<td style="background-color:#EB9900;color:white">'.$r_kbs->t_book_qty.'</td><td style="background-color:#EB9900;color:white">'.$r_kbs->t_toy_qty.'</td><td style="background-color:#EB9900;color:white">'.$r_kbs->t_other_qty.'</td>'
								.'<td style="background-color:#EB9900;color:white">'.$r_kbs->t_other_desc					
								.'</td><td style="background-color:#B5B5B5"><a href="delete.php?cluster='.$r_kbs->t_trans_no.'|'.$r_kbs->t_trans_date.'|'.$r_kbs->t_source_id.'|'.$r_kbs->kname.'|'
								.$r_kbs->t_glass_bottle_qty.'|'.$r_kbs->t_ee_qty.'|'.$r_kbs->t_comp_qty.'|'.$r_kbs->t_battery_qty.'|'.$r_kbs->t_light_qty.'|'.$r_kbs->t_paper_qty.'|'.$r_kbs->t_plastic_qty
								.'|'.$r_kbs->t_metal_qty.'|'.$r_kbs->t_toner_qty.'|'.$r_kbs->t_clothes_qty.'|'.$r_kbs->t_book_qty.'|'.$r_kbs->t_toy_qty.'|'.$r_kbs->t_other_qty.'|';
								
								//iif(!empty($r_hse->other_desc), $r_hse->other_desc, '@'); 
								if (!empty($r_kbs->t_other_desc)){
										print $r_kbs->t_other_desc;} 
										else{ 
											print '@';} 															
								
								print '"><img src="img/del.png"  width="20" height="20" value='.$r_kbs->t_trans_no.'></a></td></tr>';
	  					}
  				mysqli_free_result($result_kbs);
  			}
			echo '</table>';
