...
//	    					.'|'.$r_ot->pc_name.'|'.$r_ot->ot_qty.'|'.$r_ot->ot_unit_price.'|';
				...
				."trans_date='".$_POST['trans_date']."',station_id='".$_POST['sid']."',outlet_id='".$_POST['oid']."',prg_id='".$_POST['new_prg']."',qty=".$_POST['qty']
...
print	'</td></tr>			
			...
			<tr style="color:#555555;">
				...
				<td>  
				  <select style="margin-left:10px" name="oid" onchange="showprg(this.value)" value="'.$t[4].'">';
				 	while ($r_o=$result_o->fetch_object()){
				 		if ($r_o->outlet_id==$t[4]){
				 			print '<option value="'.$r_o->outlet_id.'" selected>'.'('.$r_o->outlet_id.') '.$r_o->name.'</option>';
				 		}else{
...
print	'</td>				  				
			...
			<tr style="color:#555555;">
				...
				<td>';  
print '<select style="margin-left:10px" name="new_prg" id="new_prg" value="'.$t[6].'">';
				 	while ($r_op=$result_op->fetch_object()){
				 		if ($r_op->op_id==$t[6]){
				 			print '<option value="'.$r_op->op_id.'" selected>'.'('.$r_op->op_id.') '.$r_op->pc_name.'</option>';
				 		}else{
				 			print '<option value="'.$r_op->op_id.'">'.'('.$r_op->op_id.') '.$r_op->pc_name.'</option>';
				 		}
				  }					  				  
print '</td>				  				
			</tr>
