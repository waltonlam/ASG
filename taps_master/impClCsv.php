    ...
    if ($_FILES["file"]["size"] > 0) {		
		...
		foreach($data as $key => $result) {		
			...
			foreach($result  as $key => $value){
				...
				if (strpos ($key,'  mg/cartridge') !== false){
					if(empty($value)){
						$conc_ppbv = '0.00';
					}else{
						$conc_ppbv_str = $value;
			...
			if ($rowcount == 0) {
				if (!empty($sampleId)){
					...
					if (!empty($res)) {
						$r_in++;
						$type = "success";
						$message = "*No. of CL records have been imported : ".$r_in;
					} else {
						$type = "error";
				...
				$count_duplicate++;
				$type = "error";
				$message = "*No. of CL records are duplicated : ".$count_duplicate;
			}
		}
...
<html>
	...
	<body>
		<h2 style="margin-left:10px">Import Carbonyls CSV File</h2><hr>
		<div id="response"
