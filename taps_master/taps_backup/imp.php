...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
        ...
        while (($column = fgetcsv($file, 20000, ",")) !== FALSE) {
            ...
            $gcde = "";
            if (isset($column[1])) {
                $gcde = mysqli_real_escape_string($dbc, $column[1]);
            }
            $sample_date = "";
            if (isset($column[2])) {
                $sample_date = mysqli_real_escape_string($dbc, $column[2]);
            }
            $m_time = "";
            if (isset($column[3])) {
                $m_time = mysqli_real_escape_string($dbc, $column[3]);
            }
            $ele_id = "";
            if (isset($column[4])) {
                $ele_id = mysqli_real_escape_string($dbc, $column[4]);
            }
            $sample_v = "";
            if (isset($column[5])) {
                $sample_v = mysqli_real_escape_string($dbc, $column[5]);
                if ($sample_v=="") $sample_v="null";
            }
            
            //$in = "INSERT into sample_ele (loc_id,gcde,sample_date,m_time,ele_id,sample_v)
            //       values ('".$loc_id."','".$gcde."',STR_TO_DATE('".$sample_date."','%d/%m/%Y')".",STR_TO_DATE('".$m_time."','%H:%i'),'".$ele_id."',".$sample_v.")";
            ...
            $res=mysqli_query($dbc, $in) or trigger_error("Query Failed! SQL: $in - Error: ".mysqli_error($dbc), E_USER_ERROR);
