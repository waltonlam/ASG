...
*/		   
			if ($_POST["review"]=="CURRENT"){
				...
				IF (glab_curr.conc_ppbv < 0,
				 ...
				 (IF (glab_curr.conc_ppbv < 0,
					(glab_curr.conc_ppbv/-2/((contractor_curr.flow_rate/1000)*contractor_curr.duration)),
					(glab_curr.conc_ppbv/((contractor_curr.flow_rate/1000)*contractor_curr.duration)) )  )*contractor_curr.who_tef  AS 'WHO TEQ Conc/m3',
				glab_curr.casno1 
...
*/
			...
			if ($_POST["review"]=="RAW"){
				...
				(IF (glab_template.conc_ppbv < 0,
					(glab_template.conc_ppbv/-2/((contractor_template.flow_rate/1000)*contractor_template.duration)),
					(glab_template.conc_ppbv/((contractor_template.flow_rate/1000)*contractor_template.duration)) )  )*contractor_template.who_tef  AS 'WHO TEQ Conc/m3',
...
*/
			...
			if ($_POST["review"]=="HISTORY"){
				...
				(IF (glab_rev_h.conc_ppbv < 0,
					(glab_rev_h.conc_ppbv/-2/((glab_rev_h.flow_rate/1000)*glab_rev_h.duration)),
					(glab_rev_h.conc_ppbv/((glab_rev_h.flow_rate/1000)*glab_rev_h.duration)) )  )*glab_rev_h.who_tef  AS 'WHO TEQ Conc/m3',
	...
	//stp				while ($r_rev = $glab_rs->fetch_array(MYSQLI_BOTH)) {
					...
					while ($r_rev = mysqli_fetch_assoc($resultunt)) {
						if ($_POST["review"]=="CURRENT"){
							...
							<td style="width:100px"><input type="number" step="0.01" id="sample_conc.'.$i.'" name="sample_conc[]" value="'.$r_rev["Conc.(ppbv)/SAMPLE"].'" style="width:100%">
							...
							<td style="width:80px"><input type="number" step="0.01" id="flow.'.$i.'" name="flow[]" value="'.$r_rev["Flow(LPM)"].'" style="width:100%" max="9999">					
							...
							<td style="width:80px"><input type="number" step="0.01" id="who_tef.'.$i.'" name="who_tef[]" value="'.$r_rev["WHO TEF Ratio"].'" style="width:100%" max="99">
							...
							<td style="width:80px"><input type="text"  id="casno1.'.$i.'" name="casno1[]" value="'.$r_rev["casno1"].'" style="width:100%">						
						...
						if ($_POST["review"]=="HISTORY"){
							...
							<td style="width:60px"><input type="number" id="conc_m3.'.$i.'" name="conc_m3[]" readonly value='.round($r_rev['Conc/M3'],5).' style="width:100%"></td>							
							<td style="width:30px"><input type="text" id="flow.'.$i.'" name="flow[]" readonly value="'.$r_rev["Flow(LPM)"].'" style="width:100%"></td>				
							<td style="width:30px"><input type="number" id="mtime.'.$i.'" name="mtime[]" readonly value='.$r_rev["Time(min)"].' style="width:100%"></td>
							<td style="width:30px"><input type="number" id="vol.'.$i.'" name="vol[]" readonly value='.round($r_rev["Volume(m3)"],5).' style="width:100%"></td>
							<td style="width:80px"><input type="number" id="who_tef.'.$i.'" name="who_tef[]" readonly value='.$r_rev["WHO TEF Ratio"].' style="width:100%"></td>
							<td style="width:30px"><input type="number" id="who_teq.'.$i.'" name="who_teq[]" readonly value='.round($r_rev["WHO TEQ Conc/m3"],5).' style="width:100%"></td>
							<td style="width:80px"><input type="text"  id="casno1.'.$i.'" name="casno1[]" readonly value="'.$r_rev['Casno1'].'" style="width:100%"></td>						
...
<!-- [Debug] 	alert('. position = ' + abc); -->
			  var rowId = $(this).attr('id').substring($(this).attr('id').indexOf('.')+1);
<!-- [Debug] alert('Attr Id = ' + $(this).attr('id').slice(0,4));	-->
<!-- [Debug] alert('rowId = ' + rowId); -->
...
<!-- [Debug]  alert ('sample_conc = ' + $(\"input[id='sample_conc.\" + rowId + \"']\").val()); -->
...
<!-- [Debug]  alert('Being modified input : ' + modi_input ); -->
			  $(this).val($(\"input[id='\"+modi_input+\"']\").first().val());

			  let result = \"input[id='rwstatus.\" + rowId + \"']\";
<!-- [Debug]   alert('Id : ' + $(this).attr('id') ); -->
<!-- [Debug]   alert('Row status id - rwresult is : ' + result ); -->
...
<!-- [Debug]   alert('Inside val function - result is : ' + result ); -->
<!-- [Debug]   alert('status oldValue is : ' + oldValue); -->
			  
<!-- [Debug]   alert ('New value : ' + \"input[id='\"+modi_input+\"']\ = \" + $(\"input[id='\"+modi_input+\"']\").first().val()); -->
<!-- [Debug]   alert ('Original value : ' + \"input[id='Original_\"+modi_input+\"']\ = \" + $(\"input[id='Original_\"+modi_input+\"']\").first().val()); -->

			  if ( $(\"input[id='Original_\"+modi_input+\"']\").first().val() === $(\"input[id='\"+modi_input+\"']\").first().val() ) {
<!-- [Debug]   alert('They are the same'); -->
...
<!-- [Debug] 	  alert('status_val = ' + status_val); -->
						  return status_val;
						  break;
					  case 'conc_m3.':
...
<!-- [Debug]	alert('They are NOT the same - return DIR'); -->
<!-- [Debug]    alert ('modi_input.replace : ' + modi_input.replace(rowId,'')) -->
